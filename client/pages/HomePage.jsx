...
const HomePage = () => {
    ...
    // Initialize state based on wallet existence to prevent UI flash
    const [checkingUser, setCheckingUser] = useState(!!walletAddress);

    useEffect(() => {
        const checkUserStatus = async () => {
            if (walletAddress) {
                // Ensure loading state is true while we check
                ...
                try {
                    ...
                    if (user) {
                        ...
                        // Connected but not registered? Go to Register
                        navigate("/register");
                    }
                } catch (err) {
                    console.error("Error checking user:", err);
                    // Optional: If error, maybe stay here or show error toast
                    setCheckingUser(false);
                }
            } else {
                // If wallet disconnects while on this page, stop loading
                setCheckingUser(false);
            }
        };
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-6 relative overflow-hidden bg-slate-950">
            ...
            <div className="absolute bottom-20 right-20 w-96 h-96 bg-emerald-500/20 rounded-full blur-[100px] pointer-events-none"></div>

            {checkingUser ? (
                // Loading State (Glass Card)
                <div className="glass-panel p-12 rounded-2xl flex flex-col items-center gap-6 animate-in fade-in zoom-in duration-300 border border-white/10 bg-slate-900/50 backdrop-blur-xl">
                    ...
                    <div className="text-center space-y-2">
                        ...
                        <p className="text-slate-400 text-sm font-mono">Querying User Registry...</p>
                    </div>
                </div>
            ) : (
                // Main Content Grid
                <div className="max-w-6xl w-full grid grid-cols-1 md:grid-cols-2 gap-12 items-center relative z-10">
                    ...
                    <div className="space-y-8 animate-in slide-in-from-left-8 duration-700">
                        ...
                        <div className="flex gap-6 pt-4">
                            ...
                            <FeatureIcon icon={<Lock />} label="AES Encryption" />
                        </div>
                    </div>
                    ...
                    <div className="animate-in slide-in-from-right-8 duration-700 delay-100">
                        <div className="glass-panel p-1 rounded-3xl shadow-2xl shadow-emerald-900/20 border border-white/10 relative group">
                            ...
                            <div className="bg-slate-900/80 backdrop-blur-xl rounded-[22px] p-8 md:p-10 text-center border border-white/5 relative">
                                <div className="w-16 h-16 bg-gradient-to-tr from-emerald-500 to-teal-400 rounded-2xl mx-auto flex items-center justify-center shadow-lg shadow-emerald-500/20 mb-6 group-hover:scale-110 transition-transform duration-300">
                                    <Activity size={32} className="text-white" />
                                </div>
                                <h2 className="text-2xl font-bold text-white mb-2">Get Started</h2>
                                <p className="text-slate-400 mb-8 text-sm leading-relaxed">
                                    Connect your MetaMask wallet to access your dashboard or register a new identity.
                                </p>
...
const FeatureIcon = ({ icon, label }) => (
    <div className="flex flex-col items-center gap-2 text-slate-500 hover:text-emerald-400 transition-colors cursor-default group">
        <div className="p-3 bg-white/5 rounded-xl border border-white/5 group-hover:bg-emerald-500/10 group-hover:border-emerald-500/20 transition-all">
